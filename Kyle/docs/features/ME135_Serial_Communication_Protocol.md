...
**[PROP-001] PROTOCOL_SPEC.md describes 400×300 (15,000 bytes) but code transmits 108×108 (1,458 bytes)** — 🟡 medium — must-fix
...
*Fix:* Update PROTOCOL_SPEC.md §3 and §4 to reflect the actual 108×108 / 1,458-byte payload. Add a note that CV runs at 400×300 internally but the protocol carries the panel-resolution matrix. Update the timing budget in §7 accordingly (~7.3 ms/frame at 2 Mbaud instead of 75 ms).
...
**[PROP-003] PROTOCOL_SPEC.md v2.0 needs to be written (not just bannered)** — 🟡 medium — must-fix
...
*Fix:* Write the actual v2.0 spec body: 512-byte payload, ~518-byte frame, 921,600 bps default. Keep v1.0 in a collapsed 'Legacy' section for reference. Update the timing budget table (512 B at 921,600 bps ≈ 5.6 ms/frame → 60+ fps theoretical).
...
**[PROP-003] No frame sequence number — impossible to detect dropped frames** — 🟢 low — future

*Problem:* The 520-byte wire protocol has sync markers, length, payload, and CRC, but no frame sequence counter. If a frame is silently dropped (ESP32 busy, serial buffer overflow), neither side can detect the gap. At 30 fps over USB, occasional drops are likely. The old PROTOCOL_SPEC.md v2 notes mention adding a frame_id but it wasn't implemented.

*Fix:* Add a 1-byte rolling frame counter (0–255) between the length field and the payload. Increment on the Python side; ESP32 logs gaps. Total frame grows from 520 to 521 bytes — negligible. Both sides need coordinated update.

**[PROP-004] PROTOCOL_SPEC.md is completely stale — describes the 108×108 WS2812B system** — 🟢 low — nice-to-have

*Problem:* agent_outputs/PROTOCOL_SPEC.md still documents the v1.0 protocol (15,000-byte payload, 2 Mbaud, Jetson, GPIO 16/17). It has a banner saying 'v2.0 should specify…' but v2.0 was never written. The actual wire protocol is now fully defined in code (serial_protocol.py + main.cpp) and partially in WIRING.md and the firmware README, but there's no single authoritative spec document.

*Fix:* Write PROTOCOL_SPEC_v2.md in Kyle/firmware/ (or Kyle/docs/) that documents the actual 520-byte frame format, 1 Mbaud USB-CDC link, CRC parameters, ACK/NAK semantics, and timing budget. Can be auto-generated from the constants in serial_protocol.py and main.cpp to stay in sync.

**[PROP-005] No integration test for CRC cross-verification** — 🟡 medium — must-fix

*Problem:* The commit message says Python and ESP32 CRC implementations were 'cross-verified' manually (crc16_ccitt('123456789')=0x29B1). But there's no automated test. If either side's CRC function is accidentally modified, the mismatch will only surface at runtime as mysterious NAK storms.

*Fix:* Add a pytest in Kyle/vision/tests/ that imports crc16_ccitt from serial_protocol.py and asserts known test vectors (b'123456789' → 0x29B1, empty → 0xFFFF, all-zeros 512-byte payload → expected value). On the firmware side, add a compile-time static_assert or a boot-time self-test that checks the same vector.

**[SERIAL-001] serial_protocol.py hardcoded 108×108 dimensions cause runtime ValueError** — 🔴 high — must-fix

*Problem:* Constants `PANEL_ROWS=108`, `PANEL_COLS=108`, `PAYLOAD_BYTES=1458` are stale. config.yaml has been updated to `output_width: 64, output_height: 64`, so the CV pipeline now emits a `(64, 64)` matrix. In `pack_matrix()`, the shape check `matrix.shape == (CV_ROWS, CV_COLS)` fails (not 300×400), then `matrix.shape != (PANEL_ROWS, PANEL_COLS)` also fails (not 108×108), raising `ValueError("Expected matrix shape (300, 400) or (108, 108), got (64, 64)")`. This is a guaranteed crash the first time `send_frame()` is called.

*Fix:* Read panel dimensions from the config dict passed to `SerialSender.__init__` instead of using module-level constants. Set `PANEL_ROWS = config['display']['panel_height']` and `PANEL_COLS = config['display']['panel_width']`, compute `PAYLOAD_BYTES` dynamically. Update `pack_matrix` to accept arbitrary shapes or to simply bit-pack whatever matrix is provided (the pipeline already resizes). Remove the stale `downsample_to_panel()` function entirely — resizing is the pipeline's responsibility.
...
**[SERIAL-001] serial_protocol.py dimensions are fatally stale — runtime crash on every frame** — 🔴 high — must-fix
...
*Fix:* Change constants to `PANEL_ROWS=64, PANEL_COLS=64, PAYLOAD_BYTES=512`. Remove the `CV_ROWS/CV_COLS` constants and the 300×400 → 108×108 downsample path entirely — the pipeline already outputs 64×64. Update `pack_matrix` to accept (64, 64) directly. Update `downsample_to_panel`, `unpack_matrix`, and all docstrings accordingly.

**[SERIAL-002] CRC-16 computed in pure-Python bit-level loop — 8× slower than necessary** — 🟢 low — nice-to-have
...
*Fix:* Replace with a 256-entry lookup table implementation (standard CRC-16/CCITT table-driven approach), or use `crcmod.predefined.mkCrcFun('crc-ccitt-false')` from the `crcmod` package. Either approach reduces per-frame CRC time to ~0.05 ms.

**[SERIAL-001] serial_protocol.py hardcodes 108×108 dimensions — breaks on 64×64 input** — 🔴 high — must-fix
...
*Fix:* Make pack_matrix() and SerialSender config-driven: read panel_width/panel_height from config['display'], compute PAYLOAD_BYTES as (w*h)//8. Remove the hardcoded constants. Update downsample_to_panel() to accept target dimensions or remove it entirely (config already outputs 64×64 natively).

**[SERIAL-002] SerialSender lacks context-manager cleanup** — 🟢 low — nice-to-have
...
*Fix:* Add `def __enter__(self): return self` and `def __exit__(self, *args): self.close(); return False` to SerialSender. Update main.py to use `with SerialSender(config) as sender:`.

**[PROTO-001] PROTOCOL_SPEC.md is self-contradictory (banner says 512B, body says 15KB)** — 🟡 medium — nice-to-have
...
*Fix:* Write the v2.0 spec body: 512-byte payload, ~518-byte total frame, 921,600 bps, CRC-16 over 512 bytes. Move the v1.0 body to a clearly labeled 'Historical: v1.0 (WS2812B)' appendix section.

**[ARCH-001] Sync serial_protocol.py payload size from 1,458 B (108×108) to 512 B (64×64)** — 🔴 high — must-fix
...
*Fix:* Change constants to PANEL_ROWS=64, PANEL_COLS=64, PAYLOAD_BYTES=512. Remove the downsample_to_panel() function (resize now happens inside the CV pipeline). Update pack_matrix() shape assertions accordingly. Update PROTOCOL_SPEC.md frame-format table (total packet 520 B, not 15,008 B).

**[ARCH-003] CVPipeline output resolution still defaults to 400×300 in code comments and CV_ROWS/CV_COLS constants** — 🟡 medium — must-fix
...
*Fix:* Remove the CV_ROWS/CV_COLS/downsample_to_panel indirection from serial_protocol.py. pack_matrix() should assert shape==(64,64) only. Update inline docstrings in cv_pipeline.py and gpu_accelerated.py that still mention '400×300 binary matrix'.

**[ARCH-005] PROTOCOL_SPEC.md frame-format table specifies v1.0 (15,008 B total) — no v2.0 exists yet** — 🟡 medium — nice-to-have
...
*Fix:* Add a PROTOCOL_SPEC v2.0 section: payload=512 B, total=520 B, baud=2,000,000 (or 921,600 both valid), timing budget updated (~2.6 ms TX, >60 fps headroom). Keep v1.0 section marked [DEPRECATED] for reference. Update §4 payload encoding to reference 64×64 matrix.
...
**[SERIAL-001] SerialSender counts retries as separate frames_sent** — 🟢 low — nice-to-have
...
*Fix:* Increment frames_sent once per send_frame() call (before the retry loop), not per transmission attempt. Add a separate retries_total counter for wire-level diagnostics.

**[PROP-04] SerialSender lacks context-manager protocol — serial port leaks on unhandled exceptions** — 🟢 low — nice-to-have
...
*Fix:* Add `__enter__` (return self) and `__exit__` (call `self.close()`) methods to `SerialSender` so it can be used as `with SerialSender(...) as sender:`. Also add `__del__` as a safety net.
...
**[PROP-004] `SerialSender` lacks context manager, risking serial port leak** — 🟢 low — nice-to-have
...
*Fix:* Add `__enter__` (return self) and `__exit__` (call `self.close()`) to `SerialSender`. Callers can then use `with SerialSender(...) as sender:` for guaranteed cleanup. Keep `close()` for backward compat.
...
**[PROTO-002] read_mode_change() races with send_mask() on the same serial port** — 🔴 high — must-fix
...
*Fix:* Add a dedicated RX thread that reads the serial port continuously and dispatches ACK bytes vs. mode-notify bytes to separate queues. _wait_ack() then reads from the ACK queue with a timeout, and the mode queue is polled from the main loop. Alternatively, add a 1-byte peek before reset_input_buffer() to drain any pending notify bytes first.
