...
**[ORCH-001] Redundant JSON roundtrip in orchestrator.py tool dispatch** — 🟢 low — nice-to-have
...
*Fix:* Replace `result = execute_tool(block.name, json.loads(json.dumps(block.input)))` with `result = execute_tool(block.name, block.input)`. If defensive copying is desired, use `copy.deepcopy(block.input)` or `dict(block.input)` instead.
