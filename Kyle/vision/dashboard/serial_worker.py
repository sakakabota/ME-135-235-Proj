"""Qt worker that owns the SerialSender on a background thread 

GUI thread emits send_mask send_tips _Core on its own QThread drains them 
SerialSender s blocking ACK wait stays off the UI thread that way 

Backpressure _send_mask_req is unbounded The GUI MUST gate on
send_complete_signal emit once wait for the signal emit again Otherwise
the queue grows when ACKs lag 30 fps camera vs 60 ms ACK 
"""
