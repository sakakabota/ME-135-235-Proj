...
- **Kyle's active pipeline** (`Kyle/vision/`, `Kyle/firmware/me135_led_pot/`): YOLOv8-seg fork of Larry's `vision.py` with Conf % trackbar → 64×64 binary mask → bit-packed serial → ESP32 → Waveshare HUB75 64×64 panel. A hardware potentiometer on the ESP32 side triggers visual effects on the panel in real time (not brightness — effects).
